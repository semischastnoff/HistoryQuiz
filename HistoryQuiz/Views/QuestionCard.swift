...
struct QuestionCard: View {
    ...
    func checkAnswer() {
        if self.index == self.correctAnswerIndex {
            ...
            self.questionsModel.currentTrainingAnswerState = true
            self.questionsModel.currentQuizAnswerState = true
        } else {
            self.answerState = .incorrect
            self.currentColor = .red
            self.questionsModel.currentTrainingAnswerState = false
            self.questionsModel.currentQuizAnswerState = false
        }
    ...
    var checkButton: some View {
        Button {
            withAnimation(.easeIn) {
                self.checkAnswer()
                self.questionsModel.didTapCheckButtonForQuiz = true
            }
        } label: {
            Text("Проверить")
